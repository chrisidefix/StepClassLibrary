...
STATEMENTPrint(Statement s, int indent_level, FILES *files) {
        ...
        switch( s->type ) {
            ...
            case STMT_ESCAPE:
                ...
                break;
            case STMT_COMPOUND:
                //fprintf(files->lib, "%*sBEGIN\n", level, "" );
                STATEMENTlist_out( s->u.compound->statements, indent_level);
                //raw( "%*sEND;\n", level, "" );
                break;
            case STMT_COND:
...
EXPRESSION__out( Expression e, int paren, int previous_op , FILES *files) {
    ...
    fprintf(stdout, "on y est dans la fonctnion\n");
    switch( TYPEis( e->type ) ) {
        case integer_:
            if( e == LITERAL_INFINITY ) {
...
EXPRESSIONop__out( struct Op_Subexpression * oe, int paren, int previous_op , FILES *files) {
    switch( oe->op_code ) {
        case OP_AND:
            fprintf(files->lib, " and ");
            break;
        case OP_ANDOR:
        case OP_OR:
            fprintf(files->lib, " or ");
            break;
        case OP_CONCAT:
        case OP_EQUAL:
